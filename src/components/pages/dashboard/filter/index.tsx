...
import { Box, Button, GridItem } from "../../../atomic";
...
export default function FilterListTransaction({}: IFilterListTransaction) {
  ...
  const onSubmit = useCallback(
    async ({ dt_init, dt_end, ...data }: IFormValues) => {
      changeFilter({
        ...filter,
        ...data,
  ...
  return (
    <Box backgroundColor="standard.white" borderRadius={8} p={"xxxs"} mt="xxxs">
      <form
        onSubmit={handleSubmit(onSubmit)}
        data-testid={"form-filter-list-transaction"}
        style={{ display: "flex" }}
      >
        ...
        <GridItem mb="xxxs" display="flex" mr="nano" alignItems="flex-end">
          <Button type="submit" variant="brand-primary-solid" size={"sm"}>
            Buscar
          </Button>
          <Button
            variant="neutral-extralight-solid"
            ml="xxxs"
