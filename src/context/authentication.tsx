...
import {
  LoginResponse,
  LoginUser,
} from "../domain/models/authentication/loginUser";
...
export interface IAuthenticationContext {
  ...
  authenticateMutate: UseMutationResult<
...
const AuthenticationProvider = ({ children }: IAuthenticationProvider) => {
  ...
  const [data, setData] = useState<AuthState | undefined>(() => {
    ...
    const { "@GOFINANCE:token": token, "@GOFINANCE:user": user } = cookies;
  ...
  const signIn = useCallback(
    async (userAuthentication: LoginUser) => {
      try {
        const response = await authenticateMutate.mutateAsync(
          userAuthentication
        );
